...
Below is an example of a Draft Answer. 
...
<ol>
  ...
  <li>If the request has not been made via the **Information Request Form** (see section *IR - Information Request Form*), the team should send this form to the customer, and request it be filled out and returned. Proceed to the next stage once it has been returned.
  </br>
  </br>
...
While teams may have their own local procedures in dealing with Information Requests, it is encouraged to have customers fill in and return the *Information Request Form* in the first instance.

This form not only aids in the team assessment stage of deadling with IRs, but also directs customers to relevant policies and protocols concerning data and statistical governance. 

A screen grab of the form can be seen below. The attachable document file (with working hyperlinks) can be found in the *Useful Documentation & Links* section of this course.
...
<div class="info_box">
  <h4>Important</h4>
  <ul>
  <li> Avoid releasing confidential data, where possible.
...
quiz(
  ...
  question("The customer is making an IR which specifically requires small numbers to be disclosed. What should you do? (select all that apply",
           ...
           answer("Do not supply the information without applying SDC.", correct = TRUE),
           answer("Release the information. It is only going to one customer, so it is fine.", correct = FALSE),
  ...
  question("All work must be checked by an appropriate member of the team (e.g. Service Manager/Principal Analyst) prior to being sent to the customer.",
...
Please take some time to give some feedback so that we may improve the course for future use.

THIS CURRENTLY IS THE FORM FOR THE INTRO R COURSE. CHANGE IT TO IRPQ COURSE

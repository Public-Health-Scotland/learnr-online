...
factor_ex
factor_ex <- factor(food, levels = c("low", "medium", "high"))
levels(factor_ex)
```
...
We're not performing another operation but to see that it works, group `borders_data` by `HospitalCode`. Use the hint button if you need some help.
...
What is the average `LengthOfStay` by `HospitalCode` in `borders_data`. Use the hint button if you need some help.
...
The `HospitalCode` "B120V" in `borders_data` needs to be changed to "B120H". Use the hint button if you need some help.
...
* Exclude the records with a missing `Dateofbirth`

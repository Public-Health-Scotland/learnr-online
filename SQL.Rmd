...
*If a table already exists with the same name, SQL will throw an error. You can suppress the error and stop the creation of the table using the `IF NOT EXISTS` clause but this isn't necessary so shown in brackets below.*
...
Let's check what that will have done to our data:

```{sql delete-output, connection="conn", exercise=TRUE, exercise.setup='prepare-drop-table'}
...
```{sql prepare-drop-table, connection="conn", exercise.setup="prepare-delete-from"}
DELETE FROM smra_test
WHERE specialty = "A77";
```

```{sql drop-table, connection="conn", exercise=TRUE, exercise.setup='prepare-drop-table'}
...
*Utilising the `.rs.askForPassword` functions provide a popup asking for details which are masked on entry and then passed back. This means that we don't have plain text passwords in our code, which we never do! We could also use `keyring`, `config`, or [environment variables](https://community.rstudio.com/t/how-to-set-a-variable-in-renviron/5029) to achieve this.*

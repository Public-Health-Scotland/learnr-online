...
knitr::include_graphics("images/renv-complete.png")
```

#### Troubleshooting

Working with `{renv}` involves more about the system you're working on and improving the ability to move projects around with confidence. This course covers the typical workflow but there's a lot of potentially new concepts and things not directly covered that could come up as you start to use the package. Here's some tips to keep you going:

* **Read any error messages** - sometimes these can appear cryptic and unrelated to your knowledge of the problem. However, they often include steps to take and functions to run to resolve the issues, or at least point you in the direction of suspicious code.
* **Follow normal troubleshooting steps** - while the issue might not be in your codebase now, the usual steps of going through the process, searching the web, and narrowing the problem scope before seeking help from colleagues are all part of a valuable skill set. 
* **Library and lockfile are out of sync** - this error message usually comes with a couple of options and it depends where you are in the workflow, but either `renv::snapshot()` when changes have been made and to update the lockfile or `renv::restore()` to set the environment back up to the specification created by the lockfile. However, a couple of other tips for this:
  * Make sure to use the `renv::install()` function, other options may be wrapped and handled by `{renv}` but options such as `remotes::install_github()` are not and may cause issues.
  * Within a shiny deployment workflow, the `{rsconnect}` package at v1.0.0 has implemented a change that is trying to work with `{renv}` but bugs have been reported. If these are affecting you, you can prevent this from happening by running `options(rsconnect.packrat = TRUE)` to turn this feature off and use the original dependency discovery.
* **Slow package installation?** - there are ongoing changes to the infrastructure to improve everyone's experience but it might be worth running the below line of code when restoring to ensure the correct package manager and type of packages are being installed to keep things running as fast as possible:

    ```R
    renv::restore(repos = c("https://ppm.publichealthscotland.org/all-r/__linux__/centos7/latest"))
    ```

```{r renv-support, echo=FALSE, fig.align='center', out.width='90%'}
knitr::include_graphics("images/illustrations/monster_support.jpg")
```
...
The instructions for geospatial packages require code to be ran each time these packages are used within a relevant project. The use of `{renv}`, therefore, is a requirement when on Posit Workbench. 
...
A major step toward automation requires reproducibility, using `{renv}` we've created the framework required to get there. While the focus has been on users working on projects, we can also use this framework with CI/CD pipelines. Using GitHub actions and the [`setup-renv` action](https://github.com/r-lib/actions/tree/v2-branch/setup-renv), steps can be added that will recreate the necessary environment on a remote server for everything from testing to deployment. More information available in [Using renv with continuous integration](https://rstudio.github.io/renv/articles/ci.html).

### Other advanced customisation

Project profiles are another abstraction that could be useful when working with `{renv}`. While we've thought of `{renv}` being useful for individual users and even up to automations on systems with CI/CD pipelines, there is also differences required in the environment for different operations or modes. This could be useful for splitting published and live versions away from development, or even project analysis from Shiny applications. More information available in [Project profiles](https://rstudio.github.io/renv/articles/profiles.html).

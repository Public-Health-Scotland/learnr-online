...
The last thing to cover when dealing with out packages is if we need to remove them. To delete packages from our project library, we use `renv::delete()` with the name of any packages to remove. 
...
quiz(
  ...
  question("What is the general process of installing a package?",
           answer("`renv::install()` > `library()` > `rev::snapshot()`", correct = TRUE),
           answer("`renv::update()` > `library()` > `rev::snapshot()`"),
           answer("`renv::install()` > `rev::snapshot()` > `library()`"),
           answer("`renv::install()` > `library()` > `rev::status()`"),
           allow_retry = TRUE,
           random_answer_order = TRUE
           )
)
...
```{r collab-quiz}
quiz(
  question("What files should **not** be committed to Git on a `{renv}` project?",

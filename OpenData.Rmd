...
### Use the CKAN R Package

The CKAN API uses web tools, however a CKAN R package, `ckanr`, has been developed that simplifies this process. If this is your first time using it, you'll need to install it. This has already been done for this application but when working on RStudio, you'll need to make sure you have the package installed, and then loaded for the session your using. To install:

```{r ckanr_install, eval=FALSE, echo=TRUE}
install.packages("ckanr")
```

### Connect to CKAN

Then, we have to set up a connection to the Scottish Health and Social Care Open Data CKAN platform. You can do this easily with a handy function from the `ckanr` package:
...
If you are interested in several resources from a dataset, it can be more efficient to download all resources in one go rather than having to extract each resource individually. This can be done by first extracting the list of resources from the dataset metadata, then looping through these resource IDs to download the data. 

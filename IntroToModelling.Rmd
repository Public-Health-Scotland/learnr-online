...
```{r, echo = FALSE, eval = TRUE, warning = FALSE, fig.align='center'}
# created a dataframe with the x and y values
data <- tibble(
  temperature_celsius = c(20, 30),
  n_ice_creams_sold = c(100, 300)
)
...
<summary>**How do we interpret correlation coefficients?** - click to expand</summary>

<br>

We've seen that the sign of $r$ has a simple meaning, but what about its magnitude (i.e. size)? J. D. Evans (1996) suggests the following scale:
...
By telling `geom_smooth()` that we want the `lm` method (i.e. linear model), we can add this best-fitting line to our plot. (Note: `se` stands for standard error and setting this to FALSE removes the fairly redundant error bar from the plot)
...
```{r residuals-quiz}
quiz(
  question("If we wanted to add a 'residuals' column to our data, what calculation would we require?",
    answer("mutate(residual = height - fit_height)", correct = TRUE),
    answer("mutate(residual = weight - fit_height)"),
    answer("mutate(residual = height + fit_height)"),
    answer("mutate(residual = height x weight)"),
    incorrect = "Not quite, have another go!",
    allow_retry = TRUE,
    random_answer_order = TRUE
  )
...
To calculate the residuals created by our line/model, we would just calculate the difference between the actual height values in our data and the fitted/predicted height values.
...
If we simply summed the residuals, the positive negative values would **cancel out**. For this reason, we tend to **square the residuals** before we sum them. This ensures no cancellation!
...
<li>The model formula, with outcome variable \~ predictor variable.</li>

<li>The minimum, medium, maximum and quartile values of the residuals. We don't need to worry about these too much.</li>
...
<li>The `Estimate` column shows us the intercept and the coefficient (gradient) for the `weight` variable, i.e. $b_0$ and $b_1$.</li>

<li>The `Std. Error` and `t value` columns, which we don't need to worry about for our purposes.</li>

<li>The `Pr(>|t|)` column shows us the p-values for the intercept and coefficient. If you're unfamiliar with this term, we will cover them shortly don't worry. For now: if p is very small, we consider this feature of the model to have a significant effect on the outcome variable.</li>
...
The highlighted components of the model summary above are the main ones to be aware of for now. We can quickly see our coefficient and whether it is considered statistically significant. R-squared at the bottom tells us
...
<li>show no systematic increase or decrease in variation across the range of data</li>
...
This plot tests the **independence** of residuals - we want to see them randomly scattered around 0 as opposed to e.g. increasing in size as the size of our fitted values increase. The blue line can help us - as long as hovers somewhat close to 0 it's fine.
...
<li>We checked our data and saw the two numeric variables, `height` and `weight`, show a linear relationship and are correlated.</li>

<li>Based on this, we built a linear regression model using `lm()`.</li>

<li>We used `summary()` to examine the p-value of the model's weight coefficient and the r-squared value. Both indicated a useful model with predictive power.</li>

<li>We used `autoplot()` to check out the diagnostic plots of the model to make sure it met the necessary assumptions.</li>

<li>These checks passed, meaning we can have confidence in using our model.</li>

</ol>

```{r lm-quiz}
quiz(
  question("When creating our model object using `lm()`, how do we structure the formula?",
    answer("formula = outcome_variable ~ predictor_variable", correct = TRUE),
    answer("formula = predictor_variable ~ outcome_variable"),
    answer("formula = outcome_variable * predictor_variable"),
    answer("formula ~ predictor_variable = outcome_variable"),
    incorrect = "Not quite, have another go!",
    allow_retry = TRUE,
    random_answer_order = TRUE
...
These include **decision trees** and **generalised linear models (GLMs)**, although these aren't covered here.
...
In reality, you may have to/want to clean the data, create some new variables based on the ones you have (known as **feature engineering**), and remove variables that are intrinsically linked to other variables so as to avoid what we call **multicollinearity** in our model. We won't cover these here, though.
...
```{r echo=FALSE, fig.align='centre', out.width="60%"}
...
```{r echo=FALSE, fig.align='centre', out.width="70%"}
...
We haven't mentioned this yet but residual standard error (RSE), shown just above R-squared in our model summary, is also a useful metric of model 'goodness-of-fit'.

We can think about it as being the 'average error' in the model's estimated response variable values - i.e. by how far on average do our fitted values differ from the actual values. Therefore, small is better. Handily, it is measured in the same units as our outcome variable.
...
Or make a prediction based on a specific set of predictor values:
...
The world of modelling is vast and we've only covered basic concepts here. There are still interactions between predictors, dealing with non-continuous numeric variables, degrees of freedom and so on.
...
-   [Transforming Publishing Team email](mailto:phs.transformingpublishing@nhs.net?subject=Introduction%20to%20R%20Training%20Online%20-%20Help)

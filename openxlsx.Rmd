...
We will now create the excel output and output the tables and chart into it.
...
Before we can add any data to the workbook we have to create worksheets for the data to go in. Note that the order you add the sheets to the workbook is the order they appear in, this is not set in code. 
...
## Excel Functionality
### Adding Pivot Tables
We can write our datasets to excel as pivot tables instead of static tables (like we did in the previous section). We'll use the second datset we created in this section:
...
writeDataTable(wb, sheet = "Data_Tables", startCol = 7, x = data_table2,
startRow = 4,

...
1.  Explain [what exactly a decision tree is](#what-is-a-dt) (with examples)

2.  [Read in the relevant data](#reading-in-data)

3.	[Wrangle the data](#data-wrangling) and perform some **feature engineering**

4.	Optional: [Create some simple plots](#visualise-data) to help identify useful predictors for our model

5.	[Build a decision tree model](#build-the-dt) using `rpart()` from the `{rpart}` package and visualise it using `rpart.plot()` from the `{rpart.plot}` package

7.	[Interpret the model](#interpret-the-dt), i.e. which variables in the data help predict survival? Which passenger demographics fare best?

8.	[Test the model’s predictive power](#testing-the-dt) by comparing predicted values to those in our data (via a **confusion matrix**, examining **sensitivity** and **specificity**)

Following this, we will [create a random forest model](#create-a-rf) and see how if this improves the predictive power we can attain.
...
## Reading in the dataset {#reading-in-data}

Click the 'Run' button below to read in our Titanic data using `read_csv()` from the `{readr}` package:
```{r readr-exercise, exercise = TRUE}
library(readr)
titanic_data <- read_csv("www/data/titanic_decision_tree_data.csv")
...
## Data wrangling and feature engineering {#data-wrangling}

Prior to building a model, cleaning and wrangling our data is always the first step. In this case, we want categorical data to be represented as such (i.e. as factors in R), as well as remove irrelevant variables and missing data to aid the model-building process.
...
## Build and plot the decision tree {#build-the-dt}
...
## Interpret the decision tree {#interpret-the-dt}
...
## Testing the decision tree {#testing-the-dt}

...
```{r intro-pathway, echo=FALSE, fig.align='center', out.width="100%", fig.cap = "source: vecteezy.com", fig.alt = "Cartoon of a man deciding which road to go down."}
...
<div class="info_box">
  <h4>Course Info</h4>
  <ul>
     <li>**Prerequisites:** [Introduction to R](https://scotland.shinyapps.io/phs-learnr-intro/), [Data Visualisation in R](https://scotland.shinyapps.io/phs-learnr-dataviz/)</li>
...
In this course, we’re going to build a decision tree based on data about passengers on-board the Titanic. This model will allow us to predict who was most likely to survive the unfortunate sinking of the vessel (apologies for the morbid nature of this data!).

To do this we’re going to do the following (you can click on each to skip to that section):

1. Look at [what a decision tree is](#what-is-a-dt)

2. [Read in the relevant data](#read-in-data)

3. [Wrangle the data](#wrangle-data) and perform some **feature engineering**

4. Optional: [Create some simple plots](#visualise-data) to help identify useful predictors for our model

5. [Build and plot a decision tree model](#build-and-plot-dt) using `rpart()` from the `{rpart}` package and  `rpart.plot()` from the `{rpart.plot}` package

6.	[Interpret the model](#interpet-dt), i.e. which variables in the data help predict survival? Which passenger demographics fare best?

7.	[Test the model’s predictive power](#test-dt) by comparing predicted values to those in our data (via a **confusion matrix**, examining **sensitivity** and **specificity**)

Following this, we will [create a random forest model](#random-forest) and see if this improves the predictive power we can attain.

<br>


## What is a decision tree? {#what-is-a-dt}
...
```{r intro-quiz1}
quiz(
  question("You’re deciding what to wear today. Which method *resembles* a decision tree?",
    answer("Wear your favourite outfit every day", correct = FALSE),
    answer("Check the weather, check your schedule, and choose an outfit based on both", correct = TRUE),
    answer("Close your eyes and grab something from the closet", correct = FALSE),
    answer("Always wear black because it matches everything", correct = FALSE),
...
Imagine you have data on many films: with information on genre, awards won, run time etc. for each and, critically, a binary (yes/no) variable on whether you would enjoying watching each movie.
...
```{r echo=FALSE, fig.align='center', out.width="75%", fig.alt = "A decision tree example showing questions regarding movies to help a user decide which movie to watch."}
...
```{r intro-quiz2}
quiz(
  question("What would the decision tree above recommend for a fantasy film with 0 Oscars, a duration of 1 hour and 43 mins, an age rating of 15, streamable at home, and no giant sandworms?",
...
```{r echo=FALSE, fig.align='center', out.width="75%", fig.alt = "A diagram showing the general layout of a decision tree: root node at the top, branching down to intermediate nodes which eventually end at lead nodes at the bottom."}
...
```{r echo=FALSE, fig.align='center', out.width="75%", fig.alt = "A diagram showing how a decision tree is interpreted upside down, i.e. a root at the top to leaves at the bottom."}
...
## Read in the data {#read-in-data}

We can read in our Titanic data using `read_csv()` from the `{readr}` package. This has been done for you but you check out the data by clicking "Run Code" below:
...
_**Note:** you won't be able to access this data from your own RStudio session so we recommend following along via the interactive coding sections in the course._
...
## Data wrangling and feature engineering {#wrangle-data}

Prior to building a model, cleaning and wrangling our data is always the first step. By cleaning up the data, creating informative new variables from existing ones and dropping NAs, we can help the decision tree make better splits, resulting in improved classification accuracy.
...
2.	Turn the relevant variables into factors (`sex`, `survived`, `pclass`, `embarked`).

      *	This way the model will identify `sex`, for example, as a factor with 2 levels as opposed to a character variable.
      
      * We'll rename `survived` to `survived_flag` and `embarked` to `port_embarkation` for readability.
...
4.	Drop any variables we don’t need.
      ...
      *	It’s important to identify and remove the variables which will be of no use inside a decision
      tree prior to building it.

      *	We’ll remove those variables that are unique to each row and can’t be grouped into meaningful
      categories: `passenger_id`, `name`, `ticket`.

      *	We’ll remove `fare` and `cabin` as the information in these is better represented in the
      categorical `pclass` variable already.

5.	Drop any remaining rows with `NA`.
...
Try having a go at the above list of cleaning/wrangling tasks on your own before looking at the answers below. It may help to tackle each task one at a time and run the `titanic_clean` code on it's own to check the output after each.

(This course will continue with the wrangled data shown in the answers below so don't worry too much if yours doesn't match!)
...
titanic_clean <- titanic_data |>
  # Filter out those without a survived flag
  filter(survived %in% ___) |>
  # Convert to factor level
  mutate(sex = as.factor(___), 
         age_status = as.factor(if_else(___, "child", "adult")),
         class = factor(
           ___, 
           levels = ___, 
         ...
         port_embarkation = as.factor(___)) |>
  # Select variables of interest
  select(sex, age_status, class, 
         port_embarkation, sib_sp, parch, survived_flag) |>
  # Drop NAs
  tidyr::drop_na()
...
Note: decision trees are a form of **supervised learning** in the world of machine learning, as our data already contains the "answers" (i.e. the `survived_flag` variable) with which we can compare our tree's predictions against.

<br>

## Visualise the data {#visualise-data}
...
*Hint: try plotting* `survived_flag` *and colour-coding by different variables (such as* `class`, `age_status`, `sex`*).*
...
## Build and plot the decision tree {#build-and-plot-dt}
...
This now gives us two data frames to work with: the larger training data `titanic_train` with which we’ll build the decision tree, and the smaller testing data `titanic_test` with which we’ll test the tree with later. It’s important that we leave this testing data aside until then.
...
It’s very unlikely that we’ll get an exact match between data frames, but in general the gap between the two will be smaller with larger data. For now, this seems like a pretty even split.
...
3) The type of variable we are interested in to be passed to the `method` argument: `class` for  predicting categorical variables (e.g. survived or not), `anova` for predicting numeric variables.

We'll call the tree model `titanic_fit`. Have a go at creating the model below (the answer is further down if you need):
...
## Interpret the decision tree {#interpet-dt}
...
* All of the training data, before being split up by any questions, is represented in the **root node** at the top: the tree predicts that `survived = No` is the most likely outcome overall, with a probability of surviving of 0.41.  The % value shown in each node represents the proportion of the data that passes through that node (here, 100%).

```{r echo=FALSE, fig.align='center', out.width="25%", fig.alt = "The root node of the newly-created decision tree."}
knitr::include_graphics("images/trees/node.jpg")
```
...
* This node asks whether `age_status = adult` and, if `yes`, leads us along a branch to a **leaf node** at the bottom left that predicts `No`, with $prob(survival) = 0.13$. 43% of the data ends up at this node.
...
* `faclen` is a numeric argument that dictates the maximum number of characters used to display the levels of each categorical variable in the tree plot (when `faclen` < word length, abbreviation is used).
...
## Testing the decision tree {#test-dt}
...
Now we will use our **test** data subset on our tree to see how well the model can predict who survived the Titanic and who did not. We can think of this data as "new" data that the model hasn't yet seen.
...
* The columns, i.e. `Truth`, represent the true numbers of survivors and non-survivors from the test data.
...
For more info on this topic, click [here](https://pediaa.com/what-is-the-difference-between-specificity-and-sensitivity/).
</div>
...
And there we go - you've built a decision tree and tested it's predictive power on test data.
...
```{r numeric-tree-exercise1-setup, include = FALSE}
titanic_clean <- titanic_data |>
  filter(survived %in% c(0,1)) |>
  mutate(sex = as.factor(sex),
         age_status = as.factor(if_else(age <= 16, "child", "adult")),
...
```{r numeric-tree-exercise1, exercise = TRUE, exercise.setup = 'numeric-tree-exercise1-setup'}
...
```{r numeric-tree-exercise1-check}
grade_this({
  pass_if(
    identical(
      .result$call,
  ...
  fail_if(
    ...
    "The data argument is incorrect. Ensure you're using the complete data object."
  )
  fail_if(
...
```{r numeric-tree-exercise2-setup, include = FALSE}
titanic_clean <- titanic_data |>
  filter(survived %in% c(0,1)) |>
  mutate(sex = as.factor(sex),
         age_status = as.factor(if_else(age <= 16, "child", "adult")),
...
```{r numeric-tree-exercise2, exercise = TRUE, exercise.setup = 'numeric-tree-exercise2-setup'}
...
* As we will see, each tree in the forest is built using a different **randomly-selected**: i) subset of the data, and ii) collection of variables.
...
(Note that using a random forest as opposed to a single decision tree comes at the cost of the **transparency** of how each prediction is made.)
...
* `mtry` tells the algorithm how many variables (selected randomly from the data) to consider for splitting.
...
<iframe width="100%" height= "2300" src= "https://forms.office.com/Pages/ResponsePage.aspx?id=veDvEDCgykuAnLXmdF5JmlMENbvENPtAs5eceNN9kYdUOVNEQTNEV1E1VDdXOThWMDNHNzFLS1dPMC4u&embed=true" frameborder= "0" marginwidth= "0" marginheight= "0" style= "border: none; max-width:100%; max-height:100vh" allowfullscreen webkitallowfullscreen mozallowfullscreen msallowfullscreen> </iframe>

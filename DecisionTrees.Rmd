...
Notice that, when interpreting a decision tree, we start at the top of the tree. Unintuitively, this top node is referred to as the **root node**. The questions/forks we use to get to a final prediction/decision are known as **intermediate nodes** and the bottom nodes are called **leaf nodes**.
...
2.	Turn the relevant variables into factors (`sex`, `survived`, `pclass`, `embarkation`)
  ...
  o	This way the model will identify `sex`, for example, as a factor with 2 levels as opposed to a         character variable
...
3.	Create an `age_status` variable which groups individuals under (and including) 16 years of age into a category called “child” category and those over 16 into a category called “adult”.
  ...
  o	This is an example of **feature engineering** in which we are creating a new child/adult variable        based on a variable we already have, `age`
...
4.	Drop any variables we don’t need
  ...
  o	We’ll remove those variables that are unique to each row and can’t be grouped into meaningful            categories: `passenger_id`, `name`, `ticket`
...
```{r tree-setup, echo = FALSE, eval = TRUE}
titanic_clean <- titanic_data %>%
  filter(survived %in% c(0,1)) %>%
  mutate(sex = as.factor(sex), 
         age_status = as.factor(if_else(age <= 16, "child", "adult")),
...
grade_this({
  ...
  fail_if(
    !identical(.result$formula, survived_flag ~ .),
    "The formula argument is incorrect. Make sure it's `target_variable ~ .`."
  )
  fail_if(
    !identical(.result$data, titanic_train),
...
For visualising trees, we use the `rpart.plot()` function from the `{rpart.plot}` package. Don't worry about all the other arguments for now, just notice that the first argument we pass to it is the tree model object itself.
...
So, our decision tree is better at predicting who did *not* survive the Titanic than it is at predicting those who *did* survive.
...
quiz(
  question("Imagine you've built a decision tree (or some other binary classification model) that predicts whether a person will choose to go holiday abroad or in their country of residence. If `holiday_abroad = \"Yes\"` is the positive outcome (i.e. the 'event'), what does a sensitivity of 92% and a specificity of 68% tell you about the model?",
    answer("The model is very good at predicting who will go abroad but only okay at predicting who will stay home.", correct = TRUE),
...
Try building a decision tree that predicts the number of siblings (+ spouse) a person on board the Titanic has (`sib_sp`). Don't worry about splitting the data and testing the model, just build it using `titanic_clean`.

```{r continuous-tree-exercise-setup, include = FALSE}
titanic_clean <- titanic_data %>%
  filter(survived %in% c(0,1)) %>%
  mutate(sex = as.factor(sex),
         age_status = as.factor(if_else(age <= 16, "child", "adult")),
...
```{r continuous-tree-exercise, exercise = TRUE, exercise.setup = 'continuous-tree-exercise-setup'}
...
```{r continuous-tree-exercise-check}
grade_this({
  pass_if(
    identical(
      .result,
  ...
  fail_if(
    !identical(.result$formula, sib_sp ~ .),
    "The formula argument is incorrect. Make sure it's `target_variable ~ .`."
  )
  fail_if(
    !identical(.result$data, titanic_clean),
    "The data argument is incorrect. Ensure you're using the appropriate dataset."
  )
  fail_if(
    .result$method != 'anova',
    "The method argument is incorrect - and remember it should be in quotation marks."
...
rpart.plot(titanic_fit_cont, # enter the name of the new model
           yesno = 2,
           fallen.leaves = TRUE,
           faclen = 6,
           digits = 2)
...
After we built a collection of decision trees that each predicts with their own algorithm, the prediction with the most 'votes' becomes our model prediction. This is generally considered more powerful and more stable than just using one tree.
...
* `mtry` tells the algorithm how many variables (selected randomly from the data set) to consider for splitting.
    * In other words: in each tree, `mtry` variables are selected at each branch, providing a lot of variation across trees.
